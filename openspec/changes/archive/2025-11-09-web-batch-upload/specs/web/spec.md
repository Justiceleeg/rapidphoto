...
The web application SHALL support uploading multiple photos (up to 100) simultaneously.

#### Scenario: Select multiple photos
- **WHEN** a user selects multiple photos using the DropZone component
...
The web application SHALL display real-time progress for batch uploads.
...
The web application SHALL connect to the SSE endpoint to receive real-time upload progress updates.

#### Scenario: Connect to SSE endpoint
- **WHEN** a batch upload is initiated
- **THEN** the application establishes an SSE connection to `/api/upload-progress/:jobId`
...
The web application SHALL handle and display failed photo uploads.
...
The web application SHALL allow users to select photos for upload.

#### Scenario: Select single photo
- **WHEN** a user selects a single photo using the DropZone component
...
The web application SHALL manage upload state using a store.
